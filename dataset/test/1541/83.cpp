...
string repeat(int n, string s){string s1 = s;for (int i = 1; i < n; i++)s += s1;return s;}
void invert(char &ch){if (ch == '0')ch = '1';else ch = '0';}
string getString(char x){string s(1, x);return s;}
template <typename t>
bool areSame(vector<t> a,int n)
{unordered_map<t,t> m;for(int i=0;i<n;i++) m[a[i]]++;if(m.size()==1)return true;else return false;}
template <typename t>
t factorial(t n)
{
    t res = 1, i;
