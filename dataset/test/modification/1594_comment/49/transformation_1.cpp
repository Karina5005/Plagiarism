...
void dfs(int st, int col, int par = -1){
    ...
    for(int i: g[st]){
        /* 'if' begin */
        if(color[i] == col)/* 'if' inside */
        {
            pos = 0; return;
        }
        /* 'if' begin */
        if(color[i] == -1) /* 'if' inside */
        dfs(i,1-col,st);
    }
}
...
void test_case() {
    ...
    for(int i=0;i<=n + m;i++) /* 'for' inside */
    g[i].clear(), color[i] = -1;
    pos = 1;
