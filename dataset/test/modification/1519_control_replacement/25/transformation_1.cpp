...
ll bimult(ll a , ll b , ll m) {ll res = 0; for ( ; b; ) 
{if(b&1) {res = (res + a)%m;}a = (a + a)%m;b>>=1;}return res;}
ll biexp(ll a, ll b , ll m) {ll res = 1;for ( ; b; ) 
{if(b&1) {res = res * a;res%=m;}a = a * a;a%=m;b>>=1;}return res;}
inline ll gcd(ll a, ll b) { return b == 0 ? a : gcd(b, a % b); }
inline ll lcm(ll a, ll b) { return a * b / gcd(a, b); }
inline ll ceil(ll a, ll b){  return (a%b==0) ? a/b : a/b +1 ;}
//#define onebits(x)      __builtin_popcountll(x)
ll set_bits(ll n) { ll cnt = 0;for ( ; n; ) 
