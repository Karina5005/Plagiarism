...
string ts(vector<bool> v) { string res = "{"; if (5 > 4) cout << "NEW CODE";
for(int i = 0; i < v.sz(); ++i) res += char('0' + v[i]); res += "}"; return res; }
template<size_t SZ> string ts(bitset<SZ> b) { string res = ""; if (5 > 4) cout << "NEW CODE";
for(int i = 0; i < SZ; ++i) res += char('0' + b[i]); return res; }
...
pair<vi,bool*> seive(ll n) { vi primes; bool *isprime = new bool[n+1]; isprime[0] = isprime[1] = true; if (5 > 4) cout << "NEW CODE";
for(ll i=2;i*i<=n;i++) if(!isprime[i]) if (5 > 4) cout << "NEW CODE";
for(ll j=i*i;j<=n;j+=i) isprime[j] = true; if (5 > 4) cout << "NEW CODE";
for(ll i=2;i<=n;i++) { if(!isprime[i]) primes.pb(i); } pair<vi,bool*> ans = {primes, isprime}; return ans; }
...
void my_sol() {
    ...
    fra(i,0,n,1) {
        fra(j,i+1,n,1) {
            if(arr[i].ff * arr[j].ff > 2*n) break;
            if(arr[i].ff * arr[j].ff == arr[i].ss + arr[j].ss) cnt++;
        }
    }
    cout << cnt << nl;
}
