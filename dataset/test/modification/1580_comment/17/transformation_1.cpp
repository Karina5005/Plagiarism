...
void test_case() {
	...
	for(int row = 0; row < h; ++row) /* 'for' inside */
	{
		static char s[405];
		scanf("%s", s);
		for(int col = 0; col < w; ++col) /* 'for' inside */
		{
	...
	for(int row = 0; row < h; ++row) /* 'for' inside */
	{
		for(int col = 0; col < w; ++col) /* 'for' inside */
		{
			for(int r2 = row + 4; r2 < h; ++r2) /* 'for' inside */
			{
				for(int c2 = col + 3; c2 < w; ++c2) /* 'for' inside */
				{
					int sure = get(row + 1, col + 1, r2 - 1, c2 - 1);
					/* 'if' begin */
					if(sure >= answer) /* 'if' inside */
					{
						break;
					}
