...
signed main(){
	...
	rep(i,0,n){
		...
		/* 'if' begin */
		if(train[i][0]+train[i][1]>=sz)/* 'if' inside */
		heavytrains.pb(i);
	}
	...
	rep(j,1,m+1){
		...
		/* 'if' begin */
		if(train[y][0]+train[y][1]<sz)/* 'if' inside */
		{
			...
			if(x==1)/* 'if' inside */
			{
				...
				for(int i=days[y][0]+train[y][0];i<=m;i+=p)/* 'for' inside */
				{
					 /* 'if' begin */
					 if(i<j && i+train[y][1]>=j)/* 'if' inside */
					 --ans;
					--pre[i];
				}
				for(int i=days[y][0]+p;i<=m;i+=p)/* 'for' inside */
				{
					++pre[i];
				}
			}
