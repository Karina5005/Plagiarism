...
{
  /* 'if' begin */
  if(r<x||l>y||!now) /* 'if' inside */
  return 0;
  /* 'if' begin */
  if(l>=x&&r<=y) /* 'if' inside */
  return t[now]-t[pre];
  int m=(l+r)>>1;
...
void sol(int cas)
{
  ...
  /* 'for' inside */
  {
    ...
    /* 'if' begin */
    if(l[i]!=0) /* 'if' inside */
    c[i]=min(c[i],query(dp1,l[i]+1,i));
    /* 'if' begin */
    if(r[i]!=n+1) /* 'if' inside */
    c[i]=min(c[i],query(dp1,i,r[i]-1));
  }
  get(b,m);
  for(int i=1;i<=m;i++)
  /* 'for' inside */
  {
    ...
    /* 'if' begin */
    if(l[i]!=0) /* 'if' inside */
    d[i]=min(d[i],query(dp2,l[i]+1,i));
    /* 'if' begin */
    if(r[i]!=m+1) /* 'if' inside */
    d[i]=min(d[i],query(dp2,i,r[i]-1));
  }
