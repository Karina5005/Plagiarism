...
signed main() {
	...
	while (t--) {
		...
		sort(m.begin(), m.end());
		bool ok = 0;
		for (int fd = 0; fd < 5; ++fd) /* 'for' inside */
		{
			for (int sd = 0; sd < 5; ++sd) /* 'for' inside */
			{
				/* 'if' begin */
				if (sd == fd) /* 'if' inside */
				continue;
				...
				{
					/* 'if' begin */
					if (m[i] & ((1 << fd)) && m[i] & (1 << sd)) /* 'if' inside */
					++c;
					else /* 'if' begin */
					if (m[i] & ((1 << fd))) /* 'if' inside */
					++a;
					else /* 'if' begin */
					if (m[i] & ((1 << sd))) /* 'if' inside */
					++b;
				}
				/* 'if' begin */
				if (a > n / 2 || b > n / 2 || a + b + c != n) /* 'if' inside */
				continue;
				ok = 1;
				break;
			}
