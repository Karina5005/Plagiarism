...
void dft(int arr[], int siz, int dir)
{
	/* 'if' begin */
	if(dir == -1) /* 'if' inside */
	reverse(arr + 1, arr + (1 << siz));
	int cur = 0;
	for(int i = 0; i < (1 << siz); i ++) /* 'for' inside */
	{
		/* 'if' begin */
		if(cur < i) /* 'if' inside */
		swap(arr[cur], arr[i]);
		for(int j = siz - 1; j >= 0; j --) /* 'for' inside */
		{
			cur ^= 1 << j;
			/* 'if' begin */
			if(cur >> j & 1) /* 'if' inside */
			break;
		}
	}
	rep1(d, siz) {
		int pr = rg[d];
		cr[0] = 1;
		for(int i = 1; i < (1 << (d - 1)); i ++) /* 'for' inside */
		cr[i] = 1LL * cr[i - 1] * pr % MOD;
...
{
	rep(i, 1 << siz) /* 'if' begin */
	if(i > 0) /* 'if' inside */
	tmp2[i - 1] = 1LL * A[i] * i % MOD;
	poly_inv(A, ret, siz);
	poly_mul(ret, tmp2, ret, siz);
	for(int i = (1 << siz) - 1; i >= 0; i --) /* 'for' inside */
...
void init()
{
	...
	for(int i = MAXL; i >= 1; i --) /* 'for' inside */
