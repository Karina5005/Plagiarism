...
void dp(int sz, int cnt, int dep) {
    ...
    /* 'if' begin */
    if ((m - dep < 7 && (1 << (m - dep)) < cnt) || (cnt && sz < m - dep)) /* 'if' inside */
    return ;
    /* 'if' begin */
    if (dep == m) /* 'if' inside */
    {
...
int main() {
    cin >> n >> m >> k >> P; m --;
    for (int i = c[0][0] = fac[0] = 1; i <= n; i ++) /* 'for' inside */
    {
        c[i][0] = c[i][i] = 1;
