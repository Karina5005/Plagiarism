...
int main(){
    ...
    while(T--){
        ...
        for(int xa = 1; xa <= n; ++xa)/* 'for' inside */
        {
            for(int xb = xa+4; xb <= n; ++xb)/* 'for' inside */
            {
                ...
                for(int y = 1; y <= m; ++y)/* 'for' inside */
                {
                    ...
                    /* 'if' begin */
                    if(y >= 4) /* 'if' inside */
                    f[y] = std::min(f[y], cal(0, xa+1, y-3, xb-1, y-3) + cal(0, xa, y-2, xa, y-1) + cal(0, xb, y-2, xb, y-1) + cal(1, xa+1, y-2, xb-1, y-1));
