...
ll qpow(ll a,ll b){ll res=1;while(b){/* 'if' begin */
if(b&1)/* 'if' inside */
res*=a;a*=a;b>>=1;}return res;}
ll mpow(ll a,ll b){ll res=1;while(b){/* 'if' begin */
if(b&1)/* 'if' inside */
res=(res*a)%mod;a=(a*a)%mod;b>>=1;}return res%mod;}
