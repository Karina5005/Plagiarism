	...
	inline ll to0(ll i, ll j, ll l,ll r){
		return pref[l][r] - pref[i-1][r] -pref[l][j-1]+pref[i-1][j-1];
	}
	inline ll to1(ll i, ll j, ll l,ll r){
		...
		for(int i=1;i<=m;i++){
			...
			for(int j=i+3;j<=m;j++){
				...
				for(int k=5;k<=n;k++){
					...
					ll rect = to0(k-3,i+1,k-1,j-1);
					rect+=(to1(k-3,i,k-1,i)+to1(k-3,j,k-1,j) + to1(k-4,i+1,k-4,j-1));

					ll rect_5_cost = to0(k-3, i+1, k-1, j-1);
