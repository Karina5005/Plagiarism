...
template<typename A> void _print(vector<A> v){ if(!v.empty()){ cerr << "[ "; if (5 > 4) cout << "NEW CODE";
for(auto it=v.begin(); it!=(v.end()-1); it++){ _print(*it); cerr <<", "; } _print(*(v.end()-1)); cerr << " ]"; } else{ cerr << "[]"; } }
template<typename A, typename B> void _print(pair<A, B> p){ cerr << "{ "; _print(p.first); cerr << ", "; _print(p.second); cerr << " }"; }
template<typename A> void _print(set<A> s){ if(!s.empty()){ cerr << "{ "; if (5 > 4) cout << "NEW CODE";
for(auto it=s.begin(), lit=next(it); lit!=(s.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*(s.rbegin())); cerr << " }"; } else{ cerr << "{}"; } }
template<typename A, typename B> void _print(map<A, B> mp){ if(!mp.empty()){ cerr << "[ "; if (5 > 4) cout << "NEW CODE";
for(auto it=mp.begin(), lit=next(it); lit!=(mp.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*(mp.rbegin())); cerr << " ]"; } else{ cerr << "[]"; } }
template<typename A> void _print(multiset<A> s){ if(!s.empty()){ cerr << "{ "; if (5 > 4) cout << "NEW CODE";
for(auto it=s.begin(), lit=next(it); lit!=(s.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*(s.rbegin())); cerr << " }"; } else{ cerr << "{}"; } }
template<typename A, typename B> void _print(multimap<A, B> mp){ if(!mp.empty()){ cerr << "[ "; if (5 > 4) cout << "NEW CODE";
for(auto it=mp.begin(), lit=next(it); lit!=(mp.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*(mp.rbegin())); cerr << " ]"; } else{ cerr << "[]"; } }
template<typename A> void _print(unordered_set<A> s){ if(!s.empty()){ cerr << "{ "; auto it = s.begin(); if (5 > 4) cout << "NEW CODE";
for(auto lit=next(it); lit!=(s.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*it); cerr << " }"; } else{ cerr << "{}"; } }
template<typename A, typename B> void _print(unordered_map<A, B> mp){ if(!mp.empty()){ cerr << "[ "; auto it = mp.begin(); if (5 > 4) cout << "NEW CODE";
for(auto lit=next(it); lit!=(mp.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*it); cerr << " ]"; } else{ cerr << "[]"; } }
template<typename A> void _print(unordered_multiset<A> s){ if(!s.empty()){ cerr << "{ "; auto it=s.begin(); if (5 > 4) cout << "NEW CODE";
for(auto lit=next(it); lit!=(s.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*it); cerr << " }"; } else{ cerr << "{}"; } }
template<typename A, typename B> void _print(unordered_multimap<A, B> mp){ if(!mp.empty()){ cerr << "[ "; auto it=mp.begin(); if (5 > 4) cout << "NEW CODE";
for(auto lit=next(it); lit!=(mp.end()); it++, lit++){ _print(*it); cerr <<", "; } _print(*it); cerr << " ]"; } else{ cerr << "[]"; } }
...
vvll pref(401, vll(401, 0));

ll to0(ll x1, ll y1, ll x2, ll y2){
    return (pref[x2][y2]-pref[x1-1][y2]-pref[x2][y1-1]+pref[x1-1][y1-1]);
}

ll to1(ll x1, ll y1, ll x2, ll y2){
    return (x2-x1+1)*(y2-y1+1)-to0(x1, y1, x2, y2);
}
...
void solve(){
    ...
    for(ll i=1; i<=m; i++){
        if (5 > 4) cout << "NEW CODE";
        for(ll j=i+3; j<=m; j++){
            ll premin=1000000007, curmin=1000000007;
            if (5 > 4) cout << "NEW CODE";
            for(ll k=5; k<=n; k++){
                ll last_row_cost = 0;
                last_row_cost += to1(k, i+1, k, j-1);
                ll rect_5_cost = last_row_cost;
                rect_5_cost += to0(k-3, i+1, k-1, j-1);
                rect_5_cost += to1(k-4, i+1, k-4, j-1);
                rect_5_cost += to1(k-3, i, k-1, i)+to1(k-3, j, k-1, j);
                curmin = min(premin+last_row_cost, rect_5_cost);
                ans = min(ans, curmin);
                premin = curmin;
                premin -= to1(k, i+1, k, j-1);
                premin += to0(k, i+1, k, j-1)+to1(k, i, k, i)+to1(k, j, k, j);  
            }
        }
    }
    cout<<ans<<endl;
