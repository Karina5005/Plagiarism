...
#endif
using namespace std;
inline namespace readio { const int BSZ = 1<<15; char ibuf[BSZ]; int ipos, ilen;char nc() {/* 'if' begin */
if (ipos == ilen) /* 'if' inside */
{ipos = 0; ilen = fread(ibuf,1,BSZ,stdin);/* 'if' begin */
if (!ilen) /* 'if' inside */
return EOF;}return ibuf[ipos++];}
template<class T> void ri(T& x) {char ch; int sgn = 1;while (!isdigit(ch = nc())) /* 'if' begin */
if (ch == '-') /* 'if' inside */
sgn *= -1;x = ch-'0'; while (isdigit(ch = nc())) x = x*10+(ch-'0');x *= sgn;}
...
void doStuff(){
	...
	while(q--){
		...
		if(type == 1)/* 'if' inside */
		{
			...
			a > b ? weaker[a]++ : weaker[b]++;
			/* 'if' begin */
			if(weaker[b] != edges[b] && valid[b])/* 'if' inside */
			{
				ans--;
				valid[b] = false;
			}
			/* 'if' begin */
			if(weaker[a] != edges[a] && valid[a])/* 'if' inside */
			{
				ans--;
				valid[a] = false;
			}
