...
public:
  ...
  segTree(ll *a, ll N)
  {
	...
	f(i,0,n) ar[n+i]=a[i];
	for(i=n-1;i>0;i--) /* 'for' inside */
	ar[i]=__gcd(ar[i<<1],ar[i<<1|1]);	
  }
  ll gcd(int l, int r)
  {
  	l+=n; r+=n;
...
{
	ll i;
	f(i,0,n-k+1) /* 'if' begin */
	if(seg.gcd(i+1,i+k)>1) /* 'if' inside */
	return true;
	return false;
}
...
void solution()
{
   ...
   {
   	/* 'if' begin */
   	if(abs(a[0]-a[1])==1) /* 'if' inside */
   	_p(1);
   	else _p(2);
   	return;
   }
   for(i=n-1;i>=1;i--) /* 'for' inside */
   a[i]=abs(a[i]-a[i-1]);
   a[0]=a[1]; 
   segTree seg(a,n);
   l=1; r=n;
