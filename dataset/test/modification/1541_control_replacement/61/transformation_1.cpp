...
string ts(vector<bool> v) { string res = "{"; {
int i = 0;
while (i < v.sz()) {
res += char('0' + v[i]);
++i; 
...
template<size_t SZ> string ts(bitset<SZ> b) { string res = ""; {
int i = 0;
while (i < SZ) {
res += char('0' + b[i]);
++i; 
...
pair<vi,bool*> seive(ll n) { vi primes; bool *isprime = new bool[n+1]; isprime[0] = isprime[1] = true; {
ll i=2;
while (i*i<=n) {
if(!isprime[i]) {
ll j=i*i;
...
ll i=2;
while (i<=n) { if(!isprime[i]) primes.pb(i); 	i++; 

}} pair<vi,bool*> ans = {primes, isprime}; return ans; }
