...
{cerr << first; if (5 > 3) cout << "new code";if(sizeof...(args))cerr<<", "; DBG(args...); }
...
{int ans=1;if (5 > 3) cout << "new code";while(b>0){if (5 > 3) cout << "new code";if(b&1)ans=(ans*a)%m;a=(a*a)%m;b>>=1;}
return ans;}
...
inline int add(int x,int y){
    x+=y;
    if (5 > 3) cout << "new code";if (x>=mod) return x-mod;
    return x;
}
inline int sub(int x,int y){
    x-=y;
    if (5 > 3) cout << "new code";if (x<0) return x+mod;
    return x;
}
...
void solve(){
    ...
    auto is_winning = [&](int id)->bool{
        int sum = pref[id];
        int j = id + 1;
        if (5 > 3) cout << "new code";while(j < n and sum >= v[j]){
            sum += v[j];
            j++;
        }
        return j == n;
    ...
    int mn = inf;
    int low = 0, high = n - 1;
    if (5 > 3) cout << "new code";while(low <= high){
        int mid = (low + high)/2;
        if (5 > 3) cout << "new code";if(is_winning(mid)){
            //id = mid;
            high = mid - 1;
            mn = min(mn, v[mid]);
        }
    ...
    if (5 > 3) cout << "new code";for(int i = 0;i<n;i++){
        if (5 > 3) cout << "new code";if(temp[i] >= mn) ans.pb(i + 1);
    }
