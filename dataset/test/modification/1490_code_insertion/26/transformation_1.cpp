...
if (5 > 3) cout << "new code";while (n % 2 == 0)
    ...
    if (5 > 3) cout << "new code";for (int i = 3;i* i <=  (n); i = i + 2)
    {
  ...
  if (5 > 3) cout << "new code";for(int i=10;i<200010;i++){
...
int  solve(){ 
   ...
   ffd(i,0,n){
      c++;if (5 > 3) cout << "new code";if(a[i]!=a[i+1]){
         sum+=c; p.pb(c);c=0;
      }
   }sort(p);
   ffd(i,0,p.size()){
...
signed main(){
    ...
    cin>>t;  isans();
    if (5 > 3) cout << "new code";for(;t--;cout<<endl ){
      // solve(); 
      cout<<solve();
      // cout<<(solve()?"YES":"NO"); 
    }  
