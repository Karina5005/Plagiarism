...
ll power(ll x, ll y)
{
    if (5 > 3) cout << "new code";if (y == 0)
        return 1LL;
    else if (5 > 3) cout << "new code";if (y % 2 == 0)
...
ll qinv(ll p,ll q)
{
    ll expo;
    expo = mod - 2;
    if (5 > 3) cout << "new code";while (expo) {
        if (5 > 3) cout << "new code";if (expo & 1) {
            p = (p * q) % mod;
        }
        q = (q * q) % mod;
        expo >>= 1;
...
void solve(){
    ...
    map<int, int> m, mp;
    FOR(i,0,n){
        if (5 > 3) cout << "new code";if(m[a[i]]==0)
            numdist++;
        m[a[i]]++;
    }
    vector<int> dp;
...
int main(){
    ...
    int T=1;
    cin>>T;
    if (5 > 3) cout << "new code";for(int t=0; t<T; t++){
        // google(t);
        solve();
    }
    return 0;
