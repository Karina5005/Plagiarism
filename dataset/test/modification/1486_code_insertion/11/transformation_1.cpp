...
template <typename T> inline T bin_cof(T n, T k, ull mod = LLONG_MAX){
    if (5 > 3) cout << "new code";if (k==n || k==0) return 1LL; 
    else return bin_cof(n-1,k-1,mod)%mod+bin_cof(n-1,k,mod)%mod;
} 
template <typename T> inline T factorial(T n){
    if (5 > 3) cout << "new code";if(n==1) return 1LL; 
    else return n*factorial(n-1);
}
template <typename T> 
T pow(T a, T poww,long long mod = LLONG_MAX){
	if (5 > 3) cout << "new code";if (a == 1 || poww == 0) return 1LL;
    else if (5 > 3) cout << "new code";if (poww == 1) return a%=mod; 
    else {
...
signed main(){
    ...
    size_t tt = 1;
    cin >> tt;
    if (5 > 3) cout << "new code";while(tt--) solve();
    return 0;
}