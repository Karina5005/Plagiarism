...
template<typename A> void _print(vector<A> v){ if(!(!v.empty())){ cerr << "[]"; } else{ cerr << "[ "; {
auto it=v.begin();
for ( ; it!=(v.end()-1); ) 
{ _print(*it); cerr <<", "; 	it++; 
...
template<typename A> void _print(set<A> s){ if(!(!s.empty())){ cerr << "{}"; } else{ cerr << "{ "; {
auto it=s.begin(), lit=next(it);
for ( ; lit!=(s.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*(s.rbegin())); cerr << " }"; } }
template<typename A, typename B> void _print(map<A, B> mp){ if(!(!mp.empty())){ cerr << "[]"; } else{ cerr << "[ "; {
auto it=mp.begin(), lit=next(it);
for ( ; lit!=(mp.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*(mp.rbegin())); cerr << " ]"; } }
template<typename A> void _print(multiset<A> s){ if(!(!s.empty())){ cerr << "{}"; } else{ cerr << "{ "; {
auto it=s.begin(), lit=next(it);
for ( ; lit!=(s.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*(s.rbegin())); cerr << " }"; } }
template<typename A, typename B> void _print(multimap<A, B> mp){ if(!(!mp.empty())){ cerr << "[]"; } else{ cerr << "[ "; {
auto it=mp.begin(), lit=next(it);
for ( ; lit!=(mp.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*(mp.rbegin())); cerr << " ]"; } }
template<typename A> void _print(unordered_set<A> s){ if(!(!s.empty())){ cerr << "{}"; } else{ cerr << "{ "; auto it = s.begin(); {
auto lit=next(it);
for ( ; lit!=(s.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*it); cerr << " }"; } }
template<typename A, typename B> void _print(unordered_map<A, B> mp){ if(!(!mp.empty())){ cerr << "[]"; } else{ cerr << "[ "; auto it = mp.begin(); {
auto lit=next(it);
for ( ; lit!=(mp.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*it); cerr << " ]"; } }
template<typename A> void _print(unordered_multiset<A> s){ if(!(!s.empty())){ cerr << "{}"; } else{ cerr << "{ "; auto it=s.begin(); {
auto lit=next(it);
for ( ; lit!=(s.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*it); cerr << " }"; } }
template<typename A, typename B> void _print(unordered_multimap<A, B> mp){ if(!(!mp.empty())){ cerr << "[]"; } else{ cerr << "[ "; auto it=mp.begin(); {
auto lit=next(it);
for ( ; lit!=(mp.end()); ) 
{ _print(*it); cerr <<", "; 	it++, lit++; 

}} _print(*it); cerr << " ]"; } }
template<typename A> void _print(stack<A> s){ if(!(!s.empty())){ cerr << "T[]B"; } else{ stack<A> t; cerr << "T[ "; for ( ; s.size() != 1; ) 
{ _print(s.top()); cerr << ", "; t.push(s.top()); s.pop(); } _print(s.top()); cerr << " ]B"; t.push(s.top()); s.pop(); for ( ; !t.empty(); ) 
{ s.push(t.top()); t.pop(); } } }
template<typename A> void _print(queue<A> q){ if(!(!q.empty())){ cerr << "F[]B"; } else{ queue<A> t; cerr << "F[ "; for ( ; q.size() != 1; ) 
{ _print(q.front()); cerr << ", "; t.push(q.front()); q.pop(); } _print(q.front()); cerr << " ]B"; t.push(q.front()); q.pop(); for ( ; !t.empty(); ) 
{ q.push(t.front()); t.pop(); } } }
template<typename A> void _print(priority_queue<A> pq){ if(!(!pq.empty())){ cerr << "F[]B"; } else{ queue<A> t; cerr << "T[ "; for ( ; pq.size() != 1; ) 
{ _print(pq.top()); cerr << ", "; t.push(pq.top()); pq.pop(); } _print(pq.top()); cerr << " ]B"; t.push(pq.top()); pq.pop(); for ( ; !t.empty(); ) 
{ pq.push(t.front()); t.pop(); } } }
template<typename A> void _print(priority_queue<A, vector<A>, greater<A>> pq){ if(!(!pq.empty())){ cerr << "F[]B"; } else{ queue<A> t; cerr << "T[ "; for ( ; pq.size() != 1; ) 
{ _print(pq.top()); cerr << ", "; t.push(pq.top()); pq.pop(); } _print(pq.top()); cerr << " ]B"; t.push(pq.top()); pq.pop(); for ( ; !t.empty(); ) 
{ pq.push(t.front()); t.pop(); } } }
