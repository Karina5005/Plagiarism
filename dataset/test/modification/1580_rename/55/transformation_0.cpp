...
void solve(){
	n=read();m=read();
	for(register int i=1;i<=n;++i){scanf("%s",str+1);for(register int j=1;j<=m;++j)mp[i][j]=(str[j]=='0');}
	for(register int j=1;j<=m;++j)for(register int i=1;i<=n;++i)s[j][i]=s[j][i-1]+(mp[i][j]^1);
	ans=n*m;for(register int i=1;i<=n-3;++i){
