...
mt19937 v_lzs(chrono::high_resolution_clock::now().time_since_epoch().count());
int inf = 1e9 + 19;
int get(int lx, int ly, int rx, int ry, vector<vector<int>> &que){
    return que[rx + 1][ry + 1] - que[rx + 1][ly] - que[lx][ry + 1] + que[lx][ly];
}
void a(){
    int ans = 20;
    int n, m;
    cin >> n >> m;
    vector<vector<int>> sl(n, vector<int>(m, 0));
    for(int i = 0;i < n;i++){
        string sw_y;
        cin >> s;
        for(int j = 0;j < m;j++){
    ...
    for(int i = 0;i < n;i++){
        for(int j = 0;j < m;j++){
            p[i + 1][j + 1] = p[i + 1][j] + p[i][j + 1] - p[i][j] + sl[i][j];
        }
    }
