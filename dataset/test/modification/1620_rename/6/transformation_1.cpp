...
int main()
{
    ...
    while(gj_b--){
        ...
        int y_jmf=0;
        vi b;
        for(int fs_r=0;fs_r<am_gn;fs_r++){
            if (fnc_h[fs_r]=='a'){
                if (y_jmf!=0){
                    b.pb(y_jmf);
                }
        ...
        ll weight[xnp];
        weight[xnp-1]=1;
        for(int cs_epa=xnp-2;cs_epa>=0;cs_epa--){
            
            if(weight[cs_epa+1]>(inf/(b[cs_epa+1]+1))){
                weight[cs_epa]=inf;
                continue;
            }
            weight[cs_epa]=weight[cs_epa+1] ;
            
            weight[cs_epa]*=(ll)(b[cs_epa+1]+1);
        }
        ...
        int a_xpe=0;
        while(a_xpe<xnp ){
            freq[a_xpe]=x/((ll)(weight[a_xpe]));
            x-=weight[a_xpe]*freq[a_xpe];
            a_xpe++;
        }
        ...
        while(a_xpe<am_gn){
            if (fnc_h[a_xpe]=='a'){
                ...
                zya++;
                while(a_xpe<am_gn and fnc_h[a_xpe]=='*'){
                    a_xpe++;
                }
            }
