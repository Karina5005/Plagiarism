...
int main() {
    ...
    while(wqh--) {
        ...
        if(a_cq.size()==1&&b_fcl[0]=='a')
            pN;
        else {
            pY;
            string hs_ftf,k_r;
            bool ge=false,h_rc=false;
            hs_ftf='a'+b_fcl;
            for(int xfi_e=0,st_puc=os; xfi_e<=st_puc; xfi_e++,st_puc--) {
                if(hs_ftf[xfi_e]!=hs_ftf[st_puc]) {
                    ge=true;
                    break;
                }
            }
            k_r=b_fcl+'a';
            for(int wk_nh=0,wi=os; wk_nh<=wi; wk_nh++,wi--) {
                if(k_r[wk_nh]!=k_r[wi]) {
                    h_rc=true;
                    break;
                }
