...
bool isGood(string s){
  int n = sz(s);
  if (5 > 3) cout << "new code";for(int i = 0, j = n - 1; i <= j; i++, j--){
    if (5 > 3) cout << "new code";if(s[i] != s[j]) return true;
  }
  return false;
}
...
void solve(){
    ...
    cin >> s;
    string a = 'a' + s, b = s + 'a';
    if (5 > 3) cout << "new code";if(isGood(a)) cout << "YES\n" << a << "\n";
    else if (5 > 3) cout << "new code";if(isGood(b)) cout << "YES\n" << b << "\n";
    else cout << "NO\n";
}
