...
bool gym(string bhw_zb, string z, ll m, ll n)
{
    if (m == 0)
        return true;
    if (n == 0)
    ...
    if (bhw_zb[m - 1] == z[n - 1])
    {
        return gym(bhw_zb, z, m - 1, n - 1);
    }

    return gym(bhw_zb, z, m, n - 1);
}
bool zja_ouj(ll n)
{
    if (n == 0)
...
ll solve()
{
    ...
    loop(i, 0, s.size() - 1)
    {
        if (gol[i] == '1' and gol[i + 1] == '1')
        {
            a = i;
            break;
        }
    }
    for (ll i = gol.size() - 1; i > 0; i--)
    {
        if (gol[i] == '0' and gol[i - 1] == '0')
        {
            b = i;
            break;
        }
